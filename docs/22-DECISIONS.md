...
| 26 | Yatay sekme menüsü (`Nav.js`) kaldırılıp yerine sol sidebar (`Sidebar.js`) getirildi; AI Security/Payment Protocol, Sistem Durumu ve Veri Yönetimi sidebar'da ayrı, daha küçük bir "ikincil" grupta tutuldu. | Premium tasarım sistemi sprinti | Yatay scroll'u tamamen kaldırmak, birincil gezinmeyi sadeleştirirken mevcut fonksiyonları silmemek. |
| 27 | Sohbete gönderilen bağlama, kayıtlı tüm projelerin isim+durum özeti (ilk 10, karakter sınırı korunarak) eklendi. | Premium tasarım sistemi sprinti | "Bütün projelerimi analiz et" gibi isteklerde AI'ın gerçek, isme bağlı kayıtları kullanabilmesi; uydurma cevap riskini azaltmak. |
