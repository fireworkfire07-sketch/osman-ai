...
import { checkRateLimit, _resetRateLimitState, MAX_REQUESTS_PER_WINDOW } from "../app/api/chat/rateLimit.js";
...
test("checkRateLimit: limit aşılınca reddeder ve bekleme süresi döner", () => {
  _resetRateLimitState();
  const now = Date.now();
  for (let i = 0; i < MAX_REQUESTS_PER_WINDOW; i++) checkRateLimit("5.6.7.8", now);
...
test("checkRateLimit: farklı IP'ler birbirinin sayacını etkilemez", () => {
  _resetRateLimitState();
  const now = Date.now();
  for (let i = 0; i < MAX_REQUESTS_PER_WINDOW; i++) checkRateLimit("9.9.9.9", now);
...
test("checkRateLimit: pencere süresi geçince sayaç sıfırlanır", () => {
  _resetRateLimitState();
  const now = Date.now();
  for (let i = 0; i < MAX_REQUESTS_PER_WINDOW; i++) checkRateLimit("2.2.2.2", now);
